...
export default function Add() {
    ...
    const [registerEndDate, setRegisterEndDate] = useState(() => {
        const newMonth = new Date(new Date().setMonth(new Date().getMonth() + 1));
        return newMonth.toISOString().split('T')[0];
    });
    ...
    const validateForm = () => {
        ...
        if (!registerStartDate.trim()) {
            setRegisterStartDateError("Date de debut de project est requis.");
            isValid = false;
        } else {
            setRegisterStartDateError("");
        }
    ...
    const add = async () => {
        if (validateForm()) {
            ...
            if (registerProfilePhoto) {
                response = await axios.post('/upload/profilePhotos', {
                    image: registerProfilePhoto,
                }, {
                    headers: {
                    ...
                    setLoading(false);
                    setregisterProfilePhotoError('Error uploading profile image:' + e)
                });
            }


            let respenseRegisterRapport = null;

            if (registerRapportAttachement) {
                respenseRegisterRapport = await axios.post('/upload/rapports', {
                    image: registerRapportAttachement,
                }, {
                    headers: {
                        'Content-Type': 'multipart/form-data',
                    },
                    ...
                    setLoading(false);
                    setRegisterRapoortAttachementError('Error uploading Rapport:', e)
                });
            }

            let respenseRegisterProject = null;

            if (registerProjectAttachement) {
                respenseRegisterProject = await axios.post('/upload/projects', {
                    image: registerProjectAttachement,
                }, {
                    headers: {
                        'Content-Type': 'multipart/form-data',
                    },
            ...
            if ((response && response.status === 200 && registerProfilePhoto != '') || (registerProfilePhoto == '')) {
                //return window.location.origin + '/uploads/profilePhotos/' + response.data.newImageUrl; 
                const urlSaved = registerProfilePhoto != '' ? '/uploads/profilePhotos/' + response.data.newImageUrl : '';
                const urlRapportSaved = registerRapportAttachement != '' ? '/uploads/rapports/' + respenseRegisterRapport.data.newImageUrl : '';
                const urlProjectSaved = registerProjectAttachement != '' ? '/uploads/projects/' + respenseRegisterProject.data.newImageUrl : '';
                ...
                axios({
                    ...
                    data: {
                        ...
                        profilePhoto: urlSaved,
                        rapport: urlRapportSaved,
                        project: urlProjectSaved
        ...
        <body className="flex justify-center h-auto min-h-screen my-11">
            <div>
                <div className="bg-white p-8 rounded-lg shadow-lg w-auto h-auto" >
                    <div>
                        <h1 className="my-6 text-center text-3xl font-extrabold text-gray-900">AJOUTER UN STAGAIRE</h1>
                    </div>
                    {ErrorMessage && (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 md:gap-5">
                            ...
                            <div className="flex flex-col items-center justify-items-center">
                                ...
                                {registerProfilePhoto ? (
                                    <div className="relative group">
                                        <img
                                            src={URL.createObjectURL(registerProfilePhoto)}
                                            alt="Profile Preview"
                                        ...
                                        <div className="absolute top-1/3 left-28 opacity-0 group-hover:opacity-100">
                                            ...
                                            <label className="profilePhoto cursor-pointer">

                                                <svg className="w-11 h-11 hover:w-12 hover:h-12" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M17 17H17.01M15.6 14H18C18.9319 14 19.3978 14 19.7654 14.1522C20.2554 14.3552 20.6448 14.7446 20.8478 15.2346C21 15.6022 21 16.0681 21 17C21 17.9319 21 18.3978 20.8478 18.7654C20.6448 19.2554 20.2554 19.6448 19.7654 19.8478C19.3978 20 18.9319 20 18 20H6C5.06812 20 4.60218 20 4.23463 19.8478C3.74458 19.6448 3.35523 19.2554 3.15224 18.7654C3 18.3978 3 17.9319 3 17C3 16.0681 3 15.6022 3.15224 15.2346C3.35523 14.7446 3.74458 14.3552 4.23463 14.1522C4.60218 14 5.06812 14 6 14H8.4M12 15V4M12 4L15 7M12 4L9 7" stroke="#64748b" strokeWidth="2" strokeLinejoin="round"></path> </g></svg>
                                    ...
                                    <label className="profilePhoto flex flex-col items-center justify-center w-64 h-64 border-2 border-gray-300 border-dashed rounded-full cursor-pointer bg-gray-50  hover:bg-gray-100">
                                        <div className="rounded-full flex flex-col items-center justify-center pt-5 pb-6">
                        ...
                        <div className="mt-4 rounded-md shadow-sm -space-y-px grid grid-cols-1 gap-2 w-auto md:w-96">
                            ...
                            <div>
                                <label className="reportAttachment flex items-center p-4 gap-3 rounded-3xl border border-gray-300 border-dashed bg-gray-50 cursor-pointer">
                                    <img className="h-16 w-auto" src="https://demo.tailus.io/images/icons/upload.webp" alt="" />.
                                    <div className="space-y-2">
                                        <h4 className="text-base font-semibold text-gray-700">Importer le rapport</h4>
                                        {registerRapportAttachement ? (
                                            <span className="text-sm text-gray-600">{registerRapportAttachement.name}</span>
                                        ) : (
                                            <span className="text-sm text-gray-500">Attachement de rapport</span>
                                        )}
                                    </div>
                                    <input
                                        // type="file" 
                                        ...
                                        type="file"
                                        //accept="image/*"
                                        hidden onChange={e => setRegisterRapoortAttachement(e.target.files[0])} />
                            ...
                            <div>
                                ...
                                <label className="projectAttachment flex items-center p-4 gap-3 rounded-3xl border border-gray-300 border-dashed bg-gray-50 cursor-pointer">
                                    <img className="h-16 w-auto" src="https://demo.tailus.io/images/icons/upload.webp" alt="" />.
                                    <div className="space-y-2">
                                        <h4 className="text-base font-semibold text-gray-700">Importer le projet</h4>
                                        {registerProjectAttachement ? (
                                            <span className="text-sm text-gray-600">{registerProjectAttachement.name}</span>
                                        ) : (
                                            <span className="text-sm text-gray-500">Attachement de projet</span>
                                        )}
                                    </div>
